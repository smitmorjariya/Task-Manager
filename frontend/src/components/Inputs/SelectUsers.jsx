...
const SelectUsers = ({ selectedUsers, setSelectedUsers }) => {
    ...
    const toggleUserSelection = (userId) => {
        setTempSelectedUsers((prev) =>
            prev.includes(userId)
                ? prev.filter((id) => id !== userId) : [...prev, userId]
        );
    };
    ...
    const selectedUserAvatars = allUsers
        .filter((user) => selectedUsers.includes(user._id)).map((user) => user.profileImageUrl);
                ...
                <div className="space-y-4 h-[60vh] overflow-y-auto">
                    {allUsers.map((user) => (
                        <div key={user._id} className="flex items-center gap-4 p-3 border-b border-gray-200">
                            ...
                            <img src={user.profileImageUrl || null} alt={user.name} className="w-10 h-10 rounded-full" />
                            
                            <div className="flex-1">
                                {/* <p className="font-medium text-gray-800 dark:text-white">{user.name}</p> */}
                                <p className="font-medium text-gray-800">{user.name}</p>
                                {/* <p className="text-[13px] text-gray-500 dark:text-white">{user.email}</p> */}
